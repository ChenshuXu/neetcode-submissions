...
from solution import TimestampedVersionedKV  # noqa: E402
...
def run_candidate(operations: Sequence[Operation]) -> Tuple[Any, ...]:
    store = TimestampedVersionedKV()
    outputs = []

    for operation in operations:
        name = operation[0]
        if name == "set" and len(operation) == 5:
            store.set(operation[1], operation[2], operation[3], operation[4])
        elif name == "get" and len(operation) == 3:
            outputs.append(store.get(operation[1], operation[2]))
        elif name == "snapshot" and len(operation) == 2:
            outputs.append(store.snapshot(operation[1]))
        else:
