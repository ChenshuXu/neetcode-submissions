...
def brute_force_minimum(root_spec: TreeSpec, max_height: int) -> int:
    ...
    for size in range(len(candidates) + 1):
        for selected in combinations(candidates, size):
            fresh_root = build_tree(root_spec)
            forest = delete_nodes_reference(fresh_root, set(selected))
...
def run_candidate(root_spec: TreeSpec, max_height: int) -> int:
    ...
    original_root = build_tree(root_spec)
    values = set(collect_values(original_root))
    if original_root is not None and original_root.val in removed:
