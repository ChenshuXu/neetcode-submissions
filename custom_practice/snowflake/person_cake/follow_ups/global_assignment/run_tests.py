...
from solution import min_total_assignment_distance  # noqa: E402
from test_cases import TEST_CASES, randomized_test_cases  # noqa: E402


def show_input(case: Case) -> str:
    people, cakes = case.args
    return f"people={list(people)!r}, cakes={list(cakes)!r}"


def run_randomized_tests(seed_count: int = 400) -> int:
    for case in randomized_test_cases(seed_count):
        try:
            actual = min_total_assignment_distance(*case.args)
