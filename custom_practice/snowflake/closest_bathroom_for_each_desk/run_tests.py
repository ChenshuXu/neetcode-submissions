...
from solution import Position, closest_bathroom_distances  # noqa: E402
from test_cases import TEST_CASES, randomized_test_cases  # noqa: E402


def run_candidate(grid: Sequence[str]) -> Tuple[Tuple[Position, int], ...]:
    distances: Dict[Position, int] = closest_bathroom_distances(grid)
...
def show_input(case: Case) -> str:
    ...
    lines = ["grid=["]
    lines.extend(f"    {row!r}," for row in grid)
    lines.append("]")
    return "\n".join(lines)
