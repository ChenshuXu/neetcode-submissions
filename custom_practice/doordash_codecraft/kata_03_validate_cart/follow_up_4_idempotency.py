"""Follow-up 4: make atomic reservations idempotent by request ID."""
...
class InventoryGateway:
    ...
    def reserve(
        ...
        with self._lock:
            ...
            self._next_reservation_id += 1
            result = ReservationResult(request_id, reservation_id, "held")
            self._requests[request_id] = (fingerprint, result)
            return result
...
class CartService:
    ...
    def reserve_cart(
        ...
        reservation = self._gateway.reserve(request_id, restaurant_id, quantities)
...
def main() -> None:
    ...
    first = gateway.reserve("request-1", "restaurant-1", {"apple": 2})
    retry = gateway.reserve("request-1", "restaurant-1", {"apple": 2})

    assert first is retry
    assert gateway.snapshot("restaurant-1")["apple"].available_quantity == 1

    try:
        gateway.reserve("request-1", "restaurant-1", {"apple": 1})
        raise AssertionError("changed payload should fail")
    except ValueError:
        pass
