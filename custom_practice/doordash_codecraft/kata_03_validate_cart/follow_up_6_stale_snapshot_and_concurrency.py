"""Follow-up 6: handle stale snapshots and concurrent last-item requests."""
...
class StaleSnapshotGateway:
    def snapshot(self, restaurant_id: str) -> dict[str, InventoryItem]:
        return {"apple": InventoryItem("apple", 1, 1, 5)}
    ...
    def reserve(
        ...
        return ReservationResult(request_id, None, "rejected", (error,))


def main() -> None:
    now = datetime(2026, 8, 28, 12, 0)

    stale_response = CartService(StaleSnapshotGateway()).reserve_cart(
        "stale-request",
        "restaurant-1",
        [CartLine("apple", 1)],
        1,
    ...
    assert stale_response.validation.errors[0].allowed_quantity == 0

    inventory = {"cake": InventoryItem("cake", 1, 1, 3)}
    gateway = InventoryGateway({"restaurant-1": inventory})
    results = [None, None]

    def attempt(index: int, request_id: str) -> None:
        results[index] = gateway.reserve(
            request_id,
            "restaurant-1",
            {"cake": 1},
            now,
    ...
    threads = [
        Thread(target=attempt, args=(0, "request-a")),
        Thread(target=attempt, args=(1, "request-b")),
    ]
    ...
    assert states == ["held", "rejected"]
    assert gateway.snapshot("restaurant-1")["cake"].available_quantity == 0
    print(states)
