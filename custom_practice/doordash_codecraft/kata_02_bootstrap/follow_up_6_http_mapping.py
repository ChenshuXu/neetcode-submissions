"""Card 6: keep Card 5 and map service results to HTTP responses."""
...
def main():
    service = BootstrapService(
        MockClient(USER),
        MockClient(ADDRESS),
        MockClient(ClientError("payment down")),
        MockClient(ORDERS),
        MockClient(CONFIG_DATA),
        payment_required=False,
        base_delay_seconds=0.01,
    )

    endpoint = BootstrapEndpoint(service)
    response = endpoint.get_bootstrap("user-1")
    invalid_response = endpoint.get_bootstrap(" ")

    assert response.status_code == 200
    assert response.body["payment"] is None
